  ...
  return (
    ...
    >
      <Link
        ...
        className="flex flex-col h-full p-5 rounded-lg border border-border bg-card hover:border-primary/40 hover:bg-accent/30 transition-all duration-300 group"
      >
        <div className="text-3xl mb-3">{category.icon}</div>
        <h3 className="font-display font-semibold text-card-foreground group-hover:text-primary transition-colors">
          {category.name[locale]}
        </h3>
        <p className="text-sm text-muted-foreground mt-1 flex-1">{category.description[locale]}</p>
